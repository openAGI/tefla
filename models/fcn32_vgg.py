...
from tefla.core.layers import conv2d, upsample2d, fully_connected, max_pool, relu, prelu, register_to_collections
...
def model(inputs, is_training, reuse, num_classes=21, batch_size=1):
    ...
    upsample_args = make_args(
        batch_norm=None, activation=relu, use_bias=False, **common_args)
    ...
    score_fr = conv2d(fc7, num_classes, filter_size=(1, 1),
                      name="vgg_19/fc21", **conv_args)

    pred = tf.argmax(score_fr, axis=3)
    input_shape = inputs.get_shape().as_list()
    upscore32 = upsample2d(score_fr, [batch_size, input_shape[1], input_shape[2], num_classes], filter_size=(64, 64), stride=(32, 32),
                           name="upscore32", w_init=initz.bilinear((64, 64, num_classes, num_classes)), **logits_args)
