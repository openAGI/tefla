...
@click.option(
    '--class_labels', default=None, show_default=True, help="Comma-separated list of class labels.")
@click.option('--data_dir', default=None, show_default=True, help="Path to input data")
    ...
    for idx, cls_files in enumerate(all_files):
      for fil in cls_files[cls_val_samples[idx]:]:
        if not os.path.isfile(os.path.join(data_dir, 'training', fil)):
          copyfile(os.path.join(data_locs[idx], fil), os.path.join(data_dir, 'training', fil))
          f.write(fil[:-4] + ',' + str(labels[idx]) + '\n')
        else:
          copyfile(
              os.path.join(data_locs[idx], fil),
              os.path.join(data_dir, 'training', 'image_' + str(idx) + fil))
          f.write('image_' + str(idx) + fil[:-4] + ',' + str(labels[idx]) + '\n')
    ...
    for idx, cls_files in enumerate(all_files):
      for fil in cls_files[:cls_val_samples[idx]]:
        if not os.path.isfile(os.path.join(data_dir, 'validation', fil)):
          copyfile(os.path.join(data_locs[idx], fil), os.path.join(data_dir, 'validation', fil))
          f.write(fil[:-4] + ',' + str(labels[idx]) + '\n')
        else:
          copyfile(
              os.path.join(data_locs[idx], fil),
              os.path.join(data_dir, 'validation', 'image_' + str(idx) + fil))
          f.write('image_' + str(idx) + fil[:-4] + ',' + str(labels[idx]) + '\n')
