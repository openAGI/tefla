...
class SegPreprocessor(Preprocessor):
    ...
    def random_image_scaling(self, image, label):
        ...
        h_new = tf.to_int32(tf.multiply(
            tf.to_float(tf.shape(image)[0]), scale))
        w_new = tf.to_int32(tf.multiply(
            tf.to_float(tf.shape(image)[1]), scale))
...
class SegPreprocessorv2(Preprocessor):
    ...
    def _pad_to_bounding_box(self, image, offset_height, offset_width, target_height,
    ...
    def _get_random_scale(self, min_scale_factor, max_scale_factor, step_size):
        ...
        scale_factors = tf.lin_space(
            min_scale_factor, max_scale_factor, num_steps)
    ...
    def _resize_to_range(self, image,
                         label=None,
                         min_size=None,
                         max_size=None,
                         factor=None,
        ...
        with tf.name_scope(scope, 'resize_to_range', [image]):
            ...
            large_height = tf.to_int32(
                tf.ceil(orig_height * large_scale_factor))
            ...
            if max_size is not None:
                ...
                small_width = tf.to_int32(
                    tf.ceil(orig_width * small_scale_factor))
                small_size = tf.stack([small_height, small_width])
                new_size = tf.cond(
    ...
    def _preprocess_image_label(self, image,
                                label,
                                crop_height,
                                crop_width,
        ...
        scale = self._get_random_scale(
            min_scale_factor, max_scale_factor, scale_factor_step_size)
        processed_image, label = self._randomly_scale_image_and_label(
        ...
        mean_pixel = tf.reshape(
            mean_pixel_fn(model_variant), [1, 1, 3])
        processed_image = self._pad_to_bounding_box(
            processed_image, 0, 0, target_height, target_width, mean_pixel)

        if label is not None:
            label = self._pad_to_bounding_box(
                label, 0, 0, target_height, target_width, ignore_label)

        # Randomly crop the image and label.
        if is_training and label is not None:
            processed_image, label = self._random_crop(
    ...
    def preprocess_for_train(self, image,
                             ...
                             ignore_label=255,
                             model_variant=None):
        return self.preprocess_image_label(image,
                                           label,
                                           crop_height,
                                           crop_width,
