...
def predict(model, training_cnf, predict_dir, weights_from, dataset_name, convert, image_size, sync,
  ...
  with tf.Graph().as_default():
    ...
    inputs = tf.placeholder(
        tf.float32, shape=(None, model.image_size[0], model.image_size[0], 3), name="input")
    end_points_D = model.discriminator(inputs, True, None, num_classes=6, batch_size=32)
    ...
    util.save_images(
        'generated_images.png', sess.run(end_points_G_val['softmax']), width=128, height=128)
