...
@click.option('--is_summary', default=False, show_default=True,
              help='Path to initial weights file.')
@click.option('--num_classes', default=1, show_default=True,
              help='Number of classes to train the model.')
@click.option('--log_file_name', default='train_gen.log', show_default=True,
              help='Log file name.')
def main(model, training_cnf, data_dir, parallel, start_epoch, weights_from, resume_lr, gpu_memory_fraction, is_summary, num_classes, log_file_name):
    ...
    trainer = GenerativeLearner(model, cnf, training_iterator=training_iter, validation_iterator=validation_iter, resume_lr=resume_lr, classification=cnf[
