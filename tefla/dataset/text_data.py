...
class CharsVocabulary(Vocabulary):
  ...
  def __init__(self, filename, max_word_length):
    ...
    chars_set |= {self.bos_char, self.eos_char, self.bow_char, self.eow_char, self.pad_char}
...
class LM1BDataset(object):
  ...
  def get_batch(self, batch_size, num_steps, pad=False, forever=True):
    return get_batch(
        self._get_sentence(forever), batch_size, num_steps, self.vocab.max_word_length, pad=pad)
