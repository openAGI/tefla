# Copyright 2017 The Tensor2Tensor Authors and 2017 Mrinal Haloi. All Rights Reserved.
#
...
class ByteTextEncoder(BaseTextEncoder):
  """Encodes each byte to an id.

  For 8-bit strings only.
  """
...
class TokenTextEncoder(BaseTextEncoder):
...
class SubwordTextEncoder(TextEncoder):
  ...
  def encode(self, raw_text):
    ...
    """
    return self._tokens_to_subtoken_ids(self.tokenizer.encode(native_to_unicode(raw_text)))
  ...
  def decode(self, subtokens):
    ...
    Returns:
      a native string
    """
    return unicode_to_native(self.tokenizer.decode(self._subtoken_ids_to_tokens(subtokens)))
  ...
  def _tokens_to_subtoken_ids(self, tokens):
    ...
    ret = []
    for token in tokens:
      ret.extend(self._escaped_token_to_subtoken_ids(_escape_token(token, self._alphabet)))
    return ret
