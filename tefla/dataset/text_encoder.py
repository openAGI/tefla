...
def _escape_token(token, alphabet):
    ...
    if not isinstance(token, six.text_type):
        raise ValueError(
            "Expected string type for token, got %s" % type(token))
...
class SubwordTextEncoder(TextEncoder):
    ...
    def encode(self, raw_text):
        ...
        return self._tokens_to_subtoken_ids(
            self.tokenizer.encode(native_to_unicode(raw_text)))
    ...
    def decode(self, subtokens):
        ...
        return unicode_to_native(
            self.tokenizer.decode(self._subtoken_ids_to_tokens(subtokens)))
    ...
    def build_to_target_size(cls,
        ...
        def bisect(min_val, max_val):
            ...
            is_ok = abs(subtokenizer.vocab_size -
