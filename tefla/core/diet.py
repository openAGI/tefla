...
from tefla.core.special_fn import fn_device_dependency, variable_ref, fn_with_custom_grad
...
class DietAdamOptimizer(DietVariableOptimizer):
  ...
  def create_slots(self, var):
    ...
    if params.factored_second_moment_accumulator and len(shape) == 2:
      slots["adam_vr"] = tf.get_variable(
          name + "_adam_vr", [shape[0], 1], trainable=False, initializer=tf.zeros_initializer())
      slots["adam_vc"] = tf.get_variable(
          name + "_adam_vc", [1, shape[1]], trainable=False, initializer=tf.zeros_initializer())
    else:
      slots["adam_v"] = tf.get_variable(
          name + "_adam_v", shape, trainable=False, initializer=tf.zeros_initializer())
    if params.beta1 != 0.0:
      slots["adam_m"] = tf.get_variable(
          name + "_adam_m", shape, trainable=False, initializer=tf.zeros_initializer())
  ...
  def update_variable(self, var, grad_var):
    ...
    if params.learning_rate_decay_scheme == "noam":
      lrate *= tf.minimum(global_step * params.learning_rate_warmup_steps**-1.5, global_step**-0.5)
    else:
    ...
    if params.factored_second_moment_accumulator and len(var.shape) == 2:
      ...
      v_update = tf.assign(slots["adam_v"],
                           slots["adam_v"] * params.beta2 + grad_squared * (1.0 - params.beta2))
    ...
    if params.beta1 != 0.0:
      m_update = tf.assign(slots["adam_m"],
                           slots["adam_m"] * params.beta1 + grad_var * (1.0 - params.beta1))
...
def _quantize(x, params, randomize=True):
  ...
  if not randomize:
    return tf.bitcast(tf.cast(x / params.quantization_scale, tf.int16), tf.float16)
