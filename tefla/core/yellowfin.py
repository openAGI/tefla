...
Thanks Jian Zhang: zjian [@] stanford [.] edu
credit: https://github.com/tensorflow/tensor2tensor
"""
...
class YellowFinOptimizer(object):
  ...
  See [Zhang et. al., 2017](https://arxiv.org/abs/1706.03471) for
  details.
  """
  ...
  def __init__(self,
    ...
    self._lr_var = tf.get_variable(
        "YF_lr", dtype=tf.float32, trainable=False, initializer=learning_rate)
    self._mu_var = tf.get_variable(
        "YF_mu", dtype=tf.float32, trainable=False, initializer=tf.constant(momentum))

    # Tuning factor for learning rates step or decaying scheme.
    self.lr_factor = tf.get_variable(
        "YF_lr_factor", dtype=tf.float32, trainable=False, initializer=tf.constant(1.0))

    # Gradient Clipping Threshold.
    if clip_thresh is not None:
      self._clip_thresh_var = tf.get_variable(
          "YF_clip_thresh", dtype=tf.float32, trainable=False, initializer=tf.constant(clip_thresh))
    else:
    ...
    self._momentum_optimizer = tf.train.MomentumOptimizer(self._lr_m, self._mu_m, use_locking, name,
                                                          use_nesterov)
    ...
    self._step = tf.get_variable(
        "YF_step", dtype=tf.int32, trainable=False, initializer=tf.constant(0))
  ...
  def _curvature_range(self):
    ...
    Returns:   h_max_t, h_min_t ops
    """
    self._curv_win = tf.get_variable(
        "curv_win",
        dtype=tf.float32,
        trainable=False,
        shape=[
  ...
  def _prepare_variables(self):
    ...
    Returns:   Grad**2, Norm, Norm**2, Mean(Norm**2) ops
    """
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    with tf.variable_scope("apply_updates"):
      ...
      if self._clip_thresh_var is not None:
        ...
        apply_grad_op = self._momentum_optimizer.apply_gradients(
            zip(self._grad, self._vars), global_step=global_step, name=name)
      else:
        apply_grad_op = self._momentum_optimizer.apply_gradients(
            zip(self._grad, self._vars), global_step=global_step, name=name)
    ...
    return tf.group(apply_grad_op, prepare_variables_op, yellowfin_op, self._increment_step_op)
