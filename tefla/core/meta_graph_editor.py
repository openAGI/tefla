...
credit: tf.contrib.meta_graph_transform
"""
...
def _get_shared_init_op(initializer_names):
  ...
  """
  return_value = initializer_names.get(tf.saved_model.constants.MAIN_OP_KEY, None)
  if not return_value:
    return_value = initializer_names.get(tf.saved_model.constants.LEGACY_INIT_OP_KEY, None)
  return str(return_value[0]) if return_value else None


def _gtt_transforms(graph_def, input_names, output_names, initializer_names, transforms):
  ...
  initializer_names_flat = sorted([k for l in initializer_names.values() for k in l])
...
def _freeze_transform(graph_def, output_names, initializer_names, saver_def, checkpoint_path):
  ...
  """
  table_initializers = initializer_names.get(tf.GraphKeys.TABLE_INITIALIZERS, [])
  shared_init_op = _get_shared_init_op(initializer_names)

  graph_def = _freeze_graph_with_def_protos(graph_def, output_names, table_initializers,
                                            shared_init_op, saver_def, checkpoint_path)
  ...
  if table_initializers:
    pruned_initializer_names[tf.GraphKeys.TABLE_INITIALIZERS] = (table_initializers)
...
def _clean_save_and_restore(graph_def, op, removed_op_names):
  ...
  for index, value in enumerate(name_op_value_tensor.string_val):
    if not _is_removed(tf.compat.as_str(value), removed_op_names):
      names.append(value)
...
def _sparsify_gather_transform(graph_def, input_names, output_names, initializer_names,
                               checkpoint_path):
  ...
  sparsify_cmd = ['sparsify_gather(%sgroup_init_node="%s")' % (input_flag, sparsify_shared_init_op)]

  starting_op_names = [node.name for node in graph_def.node]

  graph_def = _gtt_transforms(graph_def, input_names, output_names, initializer_names, sparsify_cmd)
...
def _do_transforms(graph_def,
  ...
  for t in transforms:
    if t == _FREEZE_GRAPH_TRANSFORM:
      transformed_graph_def = _gtt_transforms(transformed_graph_def, input_names, output_names,
      ...
      transformed_graph_def, transformed_initializer_names = _freeze_transform(
          transformed_graph_def, output_node_names, transformed_initializer_names, saver_def,
          checkpoint_path)
      current_gtt_transforms = []
    elif t == _SPARSIFY_GATHER_TRANSFORM:
      transformed_graph_def = _gtt_transforms(transformed_graph_def, input_names, output_names,
                                              transformed_initializer_names, current_gtt_transforms)
      transformed_graph_def, transformed_initializer_names = (_sparsify_gather_transform(
          transformed_graph_def, input_names, output_names, transformed_initializer_names,
          checkpoint_path))
      current_gtt_transforms = []
    else:
...
def _connect_to_shared_init_op(graph_def, shared_init_op_name, nodes_to_connect):
...
def _freeze_graph_with_def_protos(input_graph_def, output_node_names, initializer_names,
                                  shared_init_op_name, input_saver_def, input_checkpoint):
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      _connect_to_shared_init_op(output_graph_def, shared_init_op_name, initializer_names)
...
def _find_all_mandatory_retain_ops(base_meta_graph_def):
  ...
  """
  # TODO(n3011): implement variable stripping.
  ...
  saver_defs.extend(_get_all_protos_from_collection(base_meta_graph_def, tf.GraphKeys.SAVERS))
  for saver_def in saver_defs:
    savers = initializer_names.get(tf.GraphKeys.SAVERS, [])
    savers.extend(
        [saver_def.filename_tensor_name, saver_def.save_tensor_name, saver_def.restore_op_name])
    initializer_names[tf.GraphKeys.SAVERS] = savers

  # Variable initializers
  variable_collections = [
      tf.GraphKeys.GLOBAL_VARIABLES, tf.GraphKeys.TRAINABLE_VARIABLES,
      tf.GraphKeys.MOVING_AVERAGE_VARIABLES, tf.GraphKeys.LOCAL_VARIABLES,
  ...
  # Table initializers
  op_names = _get_all_node_names_from_collection(base_meta_graph_def,
                                                 tf.GraphKeys.TABLE_INITIALIZERS)
  if op_names:
    # Sanity check to ensure we don't overwrite dictionary entries.
    assert tf.GraphKeys.TABLE_INITIALIZERS not in initializer_names
    table_initializers = [t for t in op_names]
    initializer_names[tf.GraphKeys.TABLE_INITIALIZERS] = table_initializers

  # Various init ops
  various_init_op_collections = [
      tf.saved_model.constants.LEGACY_INIT_OP_KEY, tf.saved_model.constants.MAIN_OP_KEY,
      tf.GraphKeys.INIT_OP, tf.GraphKeys.LOCAL_INIT_OP, tf.GraphKeys.READY_OP,
      tf.GraphKeys.READY_FOR_LOCAL_INIT_OP
  ]
  for op_coll in various_init_op_collections:
    op_name = _get_single_node_name_from_collection(base_meta_graph_def, op_coll)
    if op_name:
      # Sanity check to ensure we don't overwrite dictionary entries.
...
def _add_pruned_collection(base_meta_graph_def, meta_graph_def, collection_name, removed_op_names):
  """Copy collection to the transformed MetaGraphDef, omitting removed
  items."""
  ...
  if base_collection.HasField('any_list'):
    ...
    collection.node_list.value[:] = [
        s for s in base_collection.node_list.value if not _is_removed(s, removed_op_names)
    ]
  else:
...
def _add_pruned_signature(base_meta_graph_def, meta_graph_def, signature_name, removed_op_names):
  ...
  try:
    ...
    for key in base_signature.inputs:
      _check_tensor_not_removed(base_signature.inputs[key].name, removed_op_names)
    for key in base_signature.outputs:
      _check_tensor_not_removed(base_signature.outputs[key].name, removed_op_names)
...
def _get_single_node_name_from_collection(meta_graph_def, collection_key):
  ...
  if not collection.node_list.value:
    raise ValueError('Collection {} is present but type is not node_list.'.format(collection_key))
  if len(collection.node_list.value) != 1:
    raise ValueError('Collection {} is has {} elements; expected exactly one.'.format(
...
def _get_all_protos_from_collection(meta_graph_def, collection_key):
  ...
  if not collection.bytes_list.value:
    raise ValueError('Collection {} is present but type is not bytes_list.'.format(collection_key))
  proto_type = tf.get_collection_proto_type(collection_key)
  result = []
...
def _is_removed_mentioned(s, removed_op_names):
  ...
  s_names = _re.findall(r'((?:[\/]?[a-zA-Z0-9\_]*)*)', tf.compat.as_str_any(s))
  for removed_op_name in removed_op_names:
...
def _check_tensor_not_removed(tensor_name, removed_op_names):
  ...
  if _is_removed(tensor_name, removed_op_names):
    raise ValueError('Expected Tensor, but it was removed: {}'.format(tensor_name))
...
def _add_new_inits_to_collection(meta_graph_def, updated_initializer_names):
  ...
  Raises:
    ValueError: if the tensor was removed.
  """
  # TODO(n3011): Extend this to support all collections.
  if tf.GraphKeys.TABLE_INITIALIZERS in updated_initializer_names:
    orig_table_inits = _get_all_node_names_from_collection(meta_graph_def,
                                                           tf.GraphKeys.TABLE_INITIALIZERS)
    orig_table_inits = orig_table_inits if orig_table_inits else []
    updated_table_inits = updated_initializer_names[tf.GraphKeys.TABLE_INITIALIZERS]
...
def meta_graph_editor(base_meta_graph_def,
                      input_names,
                      output_names,
                      transforms,
                      tags,
  ...
  transformed_graph_def, updated_initializer_names = _do_transforms(
      base_meta_graph_def.graph_def, input_names, output_names, initializer_names, transforms,
      base_meta_graph_def.saver_def, checkpoint_path)
  ...
  base_op_names = [tf.compat.as_str(node.name) for node in base_meta_graph_def.graph_def.node]
  retained_op_names = [tf.compat.as_str(node.name) for node in meta_graph_def.graph_def.node]
  removed_op_names = set(base_op_names) - set(retained_op_names)

  # Copy saver, excluding any pruned nodes if graph was not frozen.
  # TODO(n3011): Revisit this once the problem is addressed. Currently
  ...
  for collection_name in base_meta_graph_def.collection_def:
    _add_pruned_collection(base_meta_graph_def, meta_graph_def, collection_name, removed_op_names)
  ...
  for signature_name in base_meta_graph_def.signature_def:
    _add_pruned_signature(base_meta_graph_def, meta_graph_def, signature_name, removed_op_names)
