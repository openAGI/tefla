...
def wasserstein_discriminator_loss(discriminator_real_outputs,
  ...
  with tf.name_scope(scope, 'discriminator_wasserstein_loss',
    ...
    loss_on_real = tf.losses.compute_weighted_loss(
        discriminator_real_outputs, real_weights, scope, loss_collection=None, reduction=reduction)
...
def acgan_discriminator_loss(discriminator_real_classification_logits,
  ...
  """
  with tf.name_scope(scope, 'acgan_discriminator_loss',
                     (discriminator_real_classification_logits,
                      discriminator_gen_classification_logits, one_hot_labels)) as scope:
    loss_on_generated = tf.losses.softmax_cross_entropy(
        one_hot_labels,
        discriminator_gen_classification_logits,
        ...
        loss_collection=None,
        reduction=reduction)
    loss_on_real = tf.losses.softmax_cross_entropy(
        one_hot_labels,
        discriminator_real_classification_logits,
    ...
    if add_summaries:
      tf.summary.scalar('discriminator_gen_ac_loss', loss_on_generated)
      tf.summary.scalar('discriminator_real_ac_loss', loss_on_real)
      tf.summary.scalar('discriminator_ac_loss', loss)
...
def acgan_generator_loss(discriminator_gen_classification_logits,
  ...
  Raises:
    ...
    TypeError: if the discriminator does not output a tuple.
  """
  with tf.name_scope(scope, 'acgan_generator_loss',
                     (discriminator_gen_classification_logits, one_hot_labels)) as scope:
    ...
    if add_summaries:
      tf.summary.scalar('generator_ac_loss', loss)
...
def wasserstein_gradient_penalty(real_data,
                                 generated_data,
                                 generator_inputs,
                                 discriminator_fn,
                                 discriminator_scope,
  ...
  """
  with tf.name_scope(scope, 'wasserstein_gradient_penalty', (real_data, generated_data)) as scope:
    ...
    batch_size = differences.shape[0].value or tf.shape(differences)[0]
    alpha_shape = [batch_size] + [1] * (differences.shape.ndims - 1)
    alpha = tf.random_uniform(shape=alpha_shape)
    interpolates = real_data + (alpha * differences)

    # Clear scope so update tf are added properly.
    with tf.name_scope(None):
      # Reuse variables if variables already exists.
      with tf.variable_scope(discriminator_scope, 'gpenalty_dscope', reuse=tf.AUTO_REUSE):
    ...
    penalties = tf.square(slopes - 1.0)
    penalty = tf.losses.compute_weighted_loss(
        penalties, weights, scope=scope, loss_collection=loss_collection, reduction=reduction)
...
def minimax_generator_loss(discriminator_gen_outputs,
  ...
  Returns:
    A loss Tensor. The shape depends on `reduction`.
  """
  with tf.name_scope(scope, 'generator_minimax_loss') as scope:
...
def modified_discriminator_loss(discriminator_real_outputs,
  ...
  Returns:
    A loss Tensor. The shape depends on `reduction`.
  """
  return minimax_discriminator_loss(discriminator_real_outputs, discriminator_gen_outputs,
                                    label_smoothing, real_weights, generated_weights, scope
...
def least_squares_generator_loss(discriminator_gen_outputs,
  ...
  Returns:
    A loss Tensor. The shape depends on `reduction`.
  """
  with tf.name_scope(scope, 'lsq_generator_loss', (discriminator_gen_outputs, real_label)) as scope:
    discriminator_gen_outputs = tf.to_float(discriminator_gen_outputs)
    loss = tf.squared_difference(discriminator_gen_outputs, real_label) / 2.0
    loss = tf.losses.compute_weighted_loss(loss, weights, scope, loss_collection, reduction)
...
def least_squares_discriminator_loss(discriminator_real_outputs,
                                     discriminator_gen_outputs,
                                     real_label=1,
                                     fake_label=0,
                                     real_weights=1.0,
  ...
  Returns:
    A loss Tensor. The shape depends on `reduction`.
  """
  with tf.name_scope(scope, 'lsq_discriminator_loss',
                     (discriminator_gen_outputs, real_label)) as scope:
    discriminator_real_outputs = tf.to_float(discriminator_real_outputs)
    discriminator_gen_outputs = tf.to_float(discriminator_gen_outputs)
    discriminator_real_outputs.shape.assert_is_compatible_with(discriminator_gen_outputs.shape)

    real_losses = tf.squared_difference(discriminator_real_outputs, real_label) / 2.0
    fake_losses = tf.squared_difference(discriminator_gen_outputs, fake_label) / 2.0
  ...
  if add_summaries:
    tf.summary.scalar('discriminator_gen_lsq_loss', loss_on_generated)
    tf.summary.scalar('discriminator_real_lsq_loss', loss_on_real)
    tf.summary.scalar('discriminator_lsq_loss', loss)
...
def _validate_distributions(distributions):
  ...
  for x in distributions:
    if not isinstance(x, tf.distributions.Distribution):
      raise ValueError('`distributions` must be a list of `Distributions`. '
                       'Instead, found %s.', type(x))


def _validate_information_penalty_inputs(structured_generator_inputs, predicted_distributions):
  ...
  if len(structured_generator_inputs) != len(predicted_distributions):
    raise ValueError('`structured_generator_inputs` length %i must be the same '
                     'as `predicted_distributions` length %i.' % (len(structured_generator_inputs),
                                                                  len(predicted_distributions)))


def mutual_information_penalty(structured_generator_inputs,
  ...
  """
  _validate_information_penalty_inputs(structured_generator_inputs, predicted_distributions)

  with tf.name_scope(scope, 'mutual_information_loss') as scope:
    # Calculate the negative log-likelihood of the reconstructed noise.
    log_probs = [
        tf.reduce_mean(dist.log_prob(noise))
        for dist, noise in zip(predicted_distributions, structured_generator_inputs)
    ]
    loss = -1 * tf.losses.compute_weighted_loss(
        log_probs, weights, scope, loss_collection=loss_collection, reduction=reduction)

    if add_summaries:
      tf.summary.scalar('mutual_information_penalty', loss)
...
def _numerically_stable_global_norm(tensor_list):
  ...
  list_max = tf.reduce_max([tf.reduce_max(tf.abs(x)) for x in tensor_list if x is not None])
  return list_max * tf.global_norm([x / list_max for x in tensor_list if x is not None])


def _used_weight(weights_list):
  for weight in weights_list:
    if weight is not None:
      return tf.contrib.util.constant_value(tf.convert_to_tensor(weight))
...
def _validate_args(losses_list, weight_factor, gradient_ratio):
  ...
  if weight_factor is None and gradient_ratio is None:
    raise ValueError('`weight_factor` and `gradient_ratio` cannot both be `None.`')
...
# TODO(n3011): Add ability to pass in gradients, to avoid recomputing.
def combine_adversarial_loss(main_loss,
  ...
  with tf.name_scope(scope, 'adversarial_loss', values=[main_loss, adversarial_loss]):
    # Compute gradients if we will need them.
    if gradient_summaries or gradient_ratio is not None:
      main_loss_grad_mag = _numerically_stable_global_norm(tf.gradients(main_loss, variables))
      adv_loss_grad_mag = _numerically_stable_global_norm(tf.gradients(adversarial_loss, variables))
    ...
    if gradient_summaries:
      tf.summary.scalar('main_loss_gradients', main_loss_grad_mag)
      tf.summary.scalar('adversarial_loss_gradients', adv_loss_grad_mag)
    ...
    if _used_weight((weight_factor, gradient_ratio)) == 0:
      ...
      final_loss = (main_loss + tf.stop_gradient(weight_factor) * adversarial_loss)
    elif gradient_ratio is not None:
      grad_mag_ratio = main_loss_grad_mag / (adv_loss_grad_mag + gradient_ratio_epsilon)
      adv_coeff = grad_mag_ratio / gradient_ratio
      tf.summary.scalar('adversarial_coefficient', adv_coeff)
      final_loss = (main_loss + tf.stop_gradient(adv_coeff) * adversarial_loss)
