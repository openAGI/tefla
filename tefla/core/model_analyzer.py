...
To analyze the operations in a graph:
  ...
  tefla.core.model_analyzer.analyze_ops(tf.get_default_graph(), print_info=True)

To analyze the model variables in a graph:

  variables = tf.model_variables()
  tefla.core.model_analyzer.analyze_vars(variables, print_info=False)
"""
...
def analyze_vars(variables, print_info=False):
  ...
  for var in variables:
    ...
    if print_info:
      print(var.name, tensor_description(var), '[%d, bytes: %d]' % (var_size, var_bytes))
  if print_info:
