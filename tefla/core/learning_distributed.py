...
class DistSupervisedTrainer(Base):
    ...
    def train(self, train_op, task_id, target, dataset, dataflow, cluster_spec, is_training, start_epoch=1, reuse=None, num_replicas_to_aggregate=-1):
        ...
        with tf.device('/job:worker/task:%d' % task_id) as scope:
            with tf.device(tf.replica_device_setter(cluster=cluster_spec)):
                ...
                try:
                    ...
                    n_val_iters_per_epoch = len(
                        dataset.validation_X) // self.validation_iterator.batch_size
                ...
                while not sv.should_stop():
                    try:
                        ...
                        for iteration in range(n_iters_per_epoch):
                            images, labels = dataflow.batch_inputs(dataset, batch_size=self.cnf.get('batch_size', 32), num_preprocess_threads=self.cnf.get('num_preprocess_threads', 8))
                        ...
                        for iteration in range(n_val_iters_per_epoch):
                            val_images, val_labels = dataflow.batch_inputs(dataset, batch_size=self.cnf.get('batch_size', 32), num_preprocess_threads=self.cnf.get('num_preprocess_threads', 8))
                            ...
                            for i, (_, metric_function) in enumerate(self.validation_metrics_def):
                                metric_score = metric_function(val_labels, validation_predictions_e)
    ...
    def _setup_model_loss(self, is_chief, task_id, num_workers, is_training, scope, initial_lr=0.1, reuse=None, global_step=None, num_replicas_to_aggregate=-1):
        ...
        val_losses, val_total_loss = self._tower_loss(
