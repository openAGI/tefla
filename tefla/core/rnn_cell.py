    ...
    def __call__(self, inputs, state, scope='gru_cell'):
        ...
        with tf.variable_scope(scope):
            with tf.variable_scope("gates"):  # Reset gate and update gate.
                ...
                r, u = tf.split(value=_linear([inputs, state], 2 * self._num_units, self.reuse, b_init=self._b_init, trainable=self.trainable, name=scope), num_or_size_splits=2, axis=1)
