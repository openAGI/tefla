...
credit: github.cm/google/seq2seq
"""
...
class BeamSearchState(namedtuple("BeamSearchState", ["log_probs", "finished", "lengths"])):
...
def create_initial_beam_state(config):
  """Creates an instance of `BeamState` that can be used on the first call to
  `beam_step`.
...
def hyp_score(log_probs, sequence_lengths, config):
  ...
  length_penality_ = length_penalty(
      sequence_lengths=sequence_lengths, penalty_factor=config.length_penalty_weight)
...
def mask_probs(probs, eos_token, finished):
  """Masks log probabilities such that finished beams allocate all probability
  mass to eos. Unfinished beams remain unchanged.
  ...
  finished_row = tf.one_hot(
      eos_token, vocab_size, dtype=tf.float32, on_value=0., off_value=tf.float32.min)
...
def beam_search_step(time_, logits, beam_state, config):
  ...
  scores = hyp_score(log_probs=total_probs, sequence_lengths=new_prediction_lengths, config=config)

  scores_flat = tf.reshape(scores, [-1])
  # During the first time step we only consider the initial beam
  scores_flat = tf.cond(tf.convert_to_tensor(time_) > 0, lambda: scores_flat, lambda: scores[0])
  ...
  next_finished = tf.logical_or(
      tf.gather(beam_state.finished, next_beam_ids), tf.equal(next_word_ids, config.eos_token))
  ...
  next_state = BeamSearchState(
      log_probs=next_beam_probs, lengths=next_prediction_len, finished=next_finished)

  output = BeamSearchStepOutput(
      scores=next_beam_scores, predicted_ids=next_word_ids, beam_parent_ids=next_beam_ids)
...
def beam_search(symbols_to_logits_fn,
  ...
  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished):
    ...
    return compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs, curr_finished,
  ...
  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags):
    ...
    topk_seq, topk_log_probs, topk_scores, topk_finished = grow_topk(i, alive_seq, alive_log_probs)
    alive_seq, alive_log_probs, _ = grow_alive(topk_seq, topk_scores, topk_log_probs, topk_finished)
    ...
    return (i + 1, alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags)

  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq, finished_scores,
  ...
  (_, alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags) = tf.while_loop(
      _is_finished,
      inner_loop,
      [tf.constant(0), alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags],
      shape_invariants=[
          tf.TensorShape([]),
