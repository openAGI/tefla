...
def calc_mcc(true_pos, true_neg, false_pos, false_neg):
    ...
    try:
        ...
        temp_var2 = pow((true_pos + false_pos) * (true_pos + false_neg)
...
def calc_rmse(truth, pred):
    ...
    """
    try:
        rmse = pow(mean([pow(truth[i] - pred[i], 2)
                         for i in range(0, len(truth))]), 0.5)
        return round(rmse, 3)
    except BaseException:
        return None
...
class Evaluation():
    ...
    def read_data(self, truth_file, pred_files):
        ...
        for i in pred_files:
            ...
            file_data = file_data.merge(
                pred_data, on='id', how="inner")
        if len(file_data.columns) > 3:
            self.pred = np.array([file_data.iloc[:, i]
    ...
    def eval_classification(
        ...
        eval_list = [element.strip().lower()
                     for element in eval_list]
        if self.ensemble:
            self.pred = np.array([[literal_eval(p)
                                   for p in arr] for arr in self.pred])
        ...
        if not class_names:
            if isinstance(self.truth[0], type("str")):
                raise Exception(
                    "Either provide class names or encoded labels")
        else:
            self.truth = np.array([class_names.index(tval)
                                   for tval in self.truth])
        if not self.multilabel:
            if len(self.pred.shape) > 1:
                ...
                self.pred_max = self.pred
            classes = list(
                set(np.concatenate([self.truth, self.pred_max])))
        ...
        if plot_list:
            self.eval_plot_classification(
                plot_list, classes, true_pos, false_pos, false_neg, true_neg)
        if over_all:
            self.calc_overall()
            ...
            if plot in ["confusion matrix", "conf matrix"]:
                if self.multilabel:
                    for i, _ in enumerate(classes):
                        conf_mat = np.array(
                            [[true_pos[i], false_neg[i]], [false_pos[i], true_neg[i]]])
                        plot_conf_mat(
                            conf_mat, [classes[i], "~" + str(classes[i])])
                else:
    ...
    def eval_plot_regression(self, plot_list):
        ...
        for plot in plot_list:
            ...
            if plot in ["error bar"]:
                ...
                conf_inter = [2 * (sdev / math.sqrt(total_inst))
                              for sdev in std_dev]
    ...
    def calc_overall(self):
        ...
        overall = {k: round(v / len(self.eval_result.keys()), 3)
