...
def _statistics(x, axes):
  ...
  shift = tf.stop_gradient(tf.reduce_mean(y, axes, keep_dims=True))

  shifted_mean = tf.reduce_mean(y - shift, axes, keep_dims=True)
  mean = shifted_mean + shift
  mean_squared = tf.reduce_mean(tf.square(y), axes, keep_dims=True)

  mean = tf.squeeze(mean, axes)
  mean_squared = tf.squeeze(mean_squared, axes)
  if x.dtype == tf.float16:
    return (tf.cast(mean, tf.float16), tf.cast(mean_squared, tf.float16))
  else:
...
def _validate_call_input(tensor_list, batch_dim):
  ...
  base_shape = tf.TensorShape(_get_shape(tensor_list[0]))
...
class VBN(object):
  ...
  def __init__(self,
    ...
    self._beta = 0
    self._gamma = 1
    self._batch_axis = _validate_init_input_and_get_axis(reference_batch, batch_axis)
    ...
    with tf.variable_scope(name, 'VBN', values=[reference_batch]) as self._vs:
      ...
      self._needs_broadcasting = (sorted(self._example_reduction_axes) != list(range(ndims))[:-2])

      # Calculate the sufficient statistics for the reference batch in a way
      # that can be easily modified by additional examples.
      self._ref_mean, self._ref_mean_squares = _statistics(self._reference_batch, reduction_axes)
      ...
      ref_batch_size = _static_or_dynamic_batch_size(self._reference_batch, self._batch_axis)
      self._example_weight = 1. / (tf.to_float(ref_batch_size) + 1.)
  ...
  def _virtual_statistics(self, inputs, reduction_axes):
    ...
    vb_mean = (self._example_weight * cur_mean + self._ref_weight * self._ref_mean)
    vb_mean_sq = (self._example_weight * cur_mean_sq + self._ref_weight * self._ref_mean_squares)
  ...
  def _broadcast(self, v, broadcast_shape=None):
    ...
    if self._needs_broadcasting and v is not None:
      return tf.reshape(v, b_shape)
    return v

  def reference_batch_normalization(self):
    """Return the reference batch, but batch normalized."""
    with tf.name_scope(self._vs.name):
      return tf.nn.batch_normalization(self._reference_batch, self._broadcast(self._ref_mean),
                                       self._broadcast(self._ref_variance),
                                       self._broadcast(self._beta), self._broadcast(self._gamma),
                                       self._epsilon)
  ...
  def __call__(self, inputs):
    ...
    with tf.name_scope(self._vs.name, values=[inputs, self._reference_batch]):
      ...
      vb_mean, vb_mean_sq = self._virtual_statistics(inputs, self._example_reduction_axes)
      vb_variance = vb_mean_sq - tf.square(vb_mean)
      ...
      b_shape[self._batch_axis] = _static_or_dynamic_batch_size(inputs, self._batch_axis)
      return tf.nn.batch_normalization(inputs, self._broadcast(vb_mean, b_shape),
