...
class Schedule(object):
    ...
    time.
    This object is constructed with an instance of Config (will be
...
class Config(dict):
            ...
            if k in self:
                ...
                if isinstance(self[k], Config):
                    if not isinstance(v, dict):
                        raise TypeError(
                            'dict required for Config value, got %s' % type(v))
                    self[k].strict_update(v)
                elif isinstance(self[k], OneOf):
                    if not isinstance(v, dict):
                        raise TypeError(
                            'dict required for OneOf value, got %s' % type(v))
                    # Replace OneOf with the chosen config.
    ...
    def parse(key_val_string):
        ...
        def _pair_to_kv(pair):
            ...
            key, val = pair[:split_index].strip(
            ), pair[split_index + 1:].strip()
