...
class PreprocessUtilsTest(tf.test.TestCase):
    ...
    def testNoFlipWhenProbIsZero(self):
        ...
        with self.test_session():
            actual, is_flipped = self.segpreprocessor._flip_dim(
                [image], prob=0, dim=0)
    ...
    def testFlipMultipleImagesConsistentlyWhenProbIsOne(self):
        ...
        with self.test_session() as sess:
            image, label, is_flipped = self.segpreprocessor._flip_dim(
    ...
    def testReturnCorrectCropOfSingleImage(self):
        ...
        [cropped] = self.segpreprocessor._random_crop([image_placeholder],
                                                      crop_height,
                                                      crop_width)
    ...
    def testRandomCropMaintainsNumberOfChannels(self):
        ...
        image_placeholder = tf.placeholder(tf.int32, shape=(None, None, 3))
        [cropped] = self.segpreprocessor._random_crop(
    ...
    def testReturnConsistenCropsOfImagesInTheList(self):
        ...
        with self.test_session() as sess:
            ...
            for i in range(3):
                self.assertAllEqual(
                    cropped_image[:, :, i], cropped_labels.squeeze())

    def testDieOnRandomCropWhenImagesWithDifferentWidth(self):
        crop_height, crop_width = 2, 3
        image1 = tf.placeholder(
            tf.float32, name='image1', shape=(None, None, 3))
        image2 = tf.placeholder(
            tf.float32, name='image2', shape=(None, None, 1))
        cropped = self.segpreprocessor._random_crop(
    ...
    def testReturnPaddedImageWithNonZeroPadValue(self):
        for dtype in [np.int32, np.int64, np.float32, np.float64]:
            ...
            with self.test_session():
                ...
                padded_image = self.segpreprocessor._pad_to_bounding_box(
    ...
    def testResizeTensorsToRangeWithFactor(self):
        ...
        for i, test_shape in enumerate(test_shapes):
            ...
            with self.test_session() as session:
                ...
                self.assertEqual(
                    new_tensor_list[0].shape, expected_image_shape_list[i])
                self.assertEqual(
                    new_tensor_list[1].shape, expected_label_shape_list[i])
