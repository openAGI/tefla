...
from tefla.core.yellowfin import YellowFinOptimizer
...
class YellowFinTest(tf.test.TestCase):
  ...
  def testMeasurement(self):
    ...
    b = tf.Variable(
        np.ones([
            1,
        ], dtype=np.float32), dtype=tf.float32, name="b", trainable=True)
    x = tf.constant(
        np.ones([
            n_dim,
    ...
    with tf.Session() as sess:
      ...
      for i in range(n_iter):
        ...
        res = sess.run(
            [opt._curv_win, opt._h_max, opt._h_min, opt._grad_var, opt._dist_to_opt_avg, apply_op],
            feed_dict=feed_dict)

        g_norm_squared_avg = (0.999 * g_norm_squared_avg + 0.001 * np.sum(((i + 1) * np.ones([
            n_dim + 1,
        ]))**2))
        g_norm_avg = (0.999 * g_norm_avg + 0.001 * np.linalg.norm((i + 1) * np.ones([
            n_dim + 1,
        ])))
  ...
  def testLrMu(self):
    ...
    w_grad_val = tf.Variable(
        np.zeros([
            n_dim,
        ]), dtype=tf.float32, trainable=False)
    b_grad_val = tf.Variable(
        np.zeros([
            1,
        ]), dtype=tf.float32, trainable=False)
    apply_op = opt.apply_gradients(zip([w_grad_val, b_grad_val], tvars))
    ...
    with tf.Session() as sess:
      ...
      for i in range(n_iter):
        ...
        sess.run(tf.assign(w_grad_val, (i + 1) * np.ones([
            n_dim,
        ], dtype=np.float32)))
        sess.run(tf.assign(b_grad_val, (i + 1) * np.ones([
            1,
        ], dtype=np.float32)))

        res = sess.run([
            opt._curv_win, opt._h_max, opt._h_min, opt._grad_var, opt._dist_to_opt_avg, opt._lr_var,
            opt._mu_var, apply_op
        ])
        ...
        if i > 0:
          lr, mu = self.tuneEverything(target_dist**2, target_var, 1, target_h_min, target_h_max)
