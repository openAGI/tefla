...
from tefla.core.special_layers import embedding, gated_layer
...
def model(x, is_training, reuse):
    ...
    x1 = conv1d(x, 128, name='conv1_1', **conv_args)
    x2 = conv1d(x, 128, filter_size=4, name='conv1_2', **conv_args)
    x3 = conv1d(x, 128, filter_size=5, name='conv1_3', **conv_args)
    x = merge([x1, x2, x3], 'concat', axis=1)
    x = gated_layer(x, conv1d, 384, name='gated_layer', **conv_args)
